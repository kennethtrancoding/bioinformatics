...
class ImportService:
	...
	def import_directory(self, job_id, source_dir, samples_csv, data_dir, method, started_at):
		def offload_pair(_isolate_id, registered_paths):
			"""Send one freshly imported pair to S3 and drop the local copy.

			Runs as each pair lands, rather than once the whole folder has been
			registered: the reads are only needed locally long enough to verify and
			upload them, so releasing per pair keeps peak disk at one pair instead
			of the size of the batch. A read S3 refused stays on disk --
			backup_raw_files only returns what it confirmed -- so it is never
			missing from both places at once."""
			uploaded = self.backup_raw_files(
				job_id, [Path(path) for path in registered_paths.values()]
			)
			self.release_local_raw(job_id, *uploaded)
