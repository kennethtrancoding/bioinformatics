...
def _fake_popen(argv, **kwargs):
    return _REAL_POPEN(
        [sys.executable, "-c", f"import time; time.sleep({_RUN_SECONDS})"], **kwargs
    )
...
class TestPipelineLifecycle(Base):
    ...
    def test_run_requires_known_job(self):
        ...
        self.assertEqual(
            self.client.post("/run", data={"job_id": "ABCDEFGHJKMN"}).status_code, 404
        )
...
class TestResults(Base):
    ...
    def test_view_missing_report_404(self):
        self.assertEqual(
            self.client.get(f"/results/{self.job_id}/NOPE/view").status_code, 404
        )
...
class TestLibraries(Base):
    ...
    def test_import_cli_dry_run(self):
        ...
        proc = subprocess.run(
            [sys.executable, "-m", "workflow.lib.import_samples", str(d), "-n"],
            cwd=REAL_ROOT,
            capture_output=True,
            text=True,
        )
